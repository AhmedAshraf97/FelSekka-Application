...
module.exports = async function main(s) {
    ...
    for (var j = 0; j < Drivers.length; j++) {
        ...
        RidersDurationsinTrip.push(Riders.find(n => n.ID === Drivers[j].AssignedRiders[Drivers[j].AssignedRiders.length - 1]).TimeFromOrganizationMinutes)
        ...
        if (Drivers[j].AssignedRiders.length === 1) {
            var ExpectedFare = (lastDistance / Drivers[j].TotalDistanceCoveredToDestination) * TotalMoneyTobePaidToDriverDist +
        ...
        for (var k = Drivers[j].AssignedRiders.length - 2; k >= 0; k--) {
            ...
            lastDuration += RidersRiders[FromIndexinRidersRiders].data.find(n => n.from === Drivers[j].AssignedRiders[k + 1]).duration;
