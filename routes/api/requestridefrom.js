...
router.post('/', async (req, res) => {
    ...
    await User.findOne({ where: { id: decoded.id }}).then(user=>{
        ...
        const now = new Date();   
        //Latitude validation
        if (req.body.tolatitude == null) {
            res.status(400).send({ error: "To latitude", message: "To latitude paramter is missing" });
        } else if (((req.body.tolatitude).toString()).trim().length === 0) {
            res.status(400).send({ error: "To latitude", message: "To latitude can't be empty" });
        } else if ((typeof(req.body.tolatitude) === 'string') || ((req.body.tolatitude) instanceof String)) {
            res.status(400).send({ error: "To latitude", message: "To latitude must be a decimal" });
        }
        //Longitude validation
        else if (req.body.tolongitude == null) {
            res.status(400).send({ error: "To longitude", message: "To longitude paramter is missing" });
        } else if (((req.body.tolongitude).toString()).trim().length === 0) {
            res.status(400).send({ error: "To longitude", message: "To longitude can't be empty" });
        } else if ((typeof(req.body.tolongitude) === 'string') || ((req.body.longitude) instanceof String)) {
            res.status(400).send({ error: "To longitude", message: "To longitude must be a decimal" });
        }
