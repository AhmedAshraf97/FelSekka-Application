...
async function StepByStepReorder(AvailableDriver) {
    ...
    if (AvailableDriver.ID === 64 && AvailableDriver.AssignedRiders[AvailableDriver.AssignedRiders.length - 1] === 102) {
        var x = 7;

    }
    var durationTaken = AvailableDriver.TotalDurationTaken - RemainingToDest;
    var delta = Math.max(0.25 * AvailableDriver.MaxDuration - 0.15 * durationTaken, 10)
...
module.exports = async function main() {
    ...
    while (count != Drivers.length) {
        ...
        for (var j = 0; j < Drivers.length; j++) {
            ...
            if (Drivers[j].countRiders == 70 && Drivers[j].ID === 46) {
                var x = 5;
            }
            ...
            if (lastRiderID === DriverID) { //First Rider to be assigned
                ...
                if (WeightArray.length > 0) {
                    ...
                    for (var g = 0; g < AllDriversToRider[indexinAllDriversToRider].length; g++) {
                        ...
                        if (CurrentDriver.AssignedRiders.length > 1) {
                ...
                if (WeightArray.length > 0) {
                    ...
                    if (Drivers[j].ID == 44 && ChosenRiderID == 103) {
                        var x = 7;
                    ...
                    if (Drivers[j].countRiders == WeightArrayForRiders.length) {
                        ...
                        if (WeightArrayForRiders.length > 0) {
                            ...
                            if (ChosenRiderID == 102 && Drivers[j].ID == 43) {
                                var x = 9;
                            }
        ...
        for (var j = 0; j < Drivers.length; j++) {
            ...
            if (Drivers[j].AssignedRiders[Drivers[j].AssignedRiders.length - 1] === Drivers[j].ID) // last rider driver
            {
                ...
                if (indexinDriverRider === -1) {
                    count++;
                } else if (DriversRiders[indexinDriverRider].checked === DriversRiders[indexinDriverRider].length || (Drivers[j].AssignedRiders.length - 1) === Drivers[j].capacity) {
                    count++;
