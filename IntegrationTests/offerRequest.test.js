...
test('offer to-organization trip', async() => {
    var decoded = jwt.verify(CurrentToken2, process.env.SECRET_KEY)
    const response = await request(app)
...
test('offer to-organization trip at the same time', async() => {
    const response = await request(app)
        ...
        .set('Authorization', CurrentToken2).expect(400)
...
test('Show pending offers', async() => {
    const response = await request(app)
        .post('/api/showoffers')
        .set('Authorization', CurrentToken2).expect(200)
})
...
test('Show pending offers after cancellation', async() => {
    const response = await request(app)
        .post('/api/showoffers')
        .set('Authorization', CurrentToken2).expect(409)
    expect(response.body.message).toBe("No pending offers")
})


test('Show pending requests', async() => {
    const response = await request(app)
        .post('/api/showrequests')
        .set('Authorization', CurrentToken).expect(200)
...
test('Show pending requests after cancellation', async() => {
    const response = await request(app)
        .post('/api/showrequests')
        .set('Authorization', CurrentToken).expect(409)
    expect(response.body.message).toBe("No pending requests")
})